...
#Toy Dataset
for hidden_layer in [5, 1, 2, 3, 10, 20, 40]:
	layers_dim = [2]+[hidden_layer]+[1]
	print "Starting "+str(layers_dim)
	cf, tr, te, weights = train_network_single_eg(X, y, X_test, y_test, layers_dim, len(X), activation_unit='sigmoid')
	print(tr)
	print(te)
	plt.figure()
	...
	plot_decision_boundary(return_output, X, y, '1contour_train_'+str(hidden_layer)+'.png')
	plot_decision_boundary(return_output, X_test, y_test, '1contour_test_'+str(hidden_layer)+'.png')
